...
pub fn Route(comptime Server: type, comptime UserState: type) type {
    return struct {
        ...
        pub const PrepareFn = *const fn (self: *Self, allocator: std.mem.Allocator) anyerror!void;
        pub const TeardownFn = *const fn (self: *Self, allocator: std.mem.Allocator) void;

        /// The defined preparation function, which can allocate custom route data.
        /// It is called at runtime when the router is initialized.
        prepare: ?PrepareFn = null,

        /// The defined teardown function, which can free custom route data.
        /// It is called at runtime when the router is deinitialized.
        teardown: ?TeardownFn = null,

        /// Runtime custom route data, if there are some.
        data: ?*anyopaque = null,
        ...
        /// Set a preparation function to be called at runtime, which can allocate custom route data.
        /// Also set the corresponding teardown function, if required.
        pub fn set_prepare(self: Self, prepareFn: PrepareFn, teardownFn: ?TeardownFn) Self {
            return Self{
                .path = self.path,
                .handlers = self.handlers,
                .prepare = prepareFn,
        ...
        pub fn serve_fs_dir(comptime _self: *const Self, comptime dir_path: []const u8) Self {
            ...
            return _self
                ...
                // Add preparations to get the real path at runtime.
                .set_prepare(struct {
                    fn f(self: *Self, allocator: std.mem.Allocator) !void {
                        const slice = try allocator.create([]const u8);
                        // Gets the real path of the directory being served.
                        slice.* = try std.fs.realpathAlloc(allocator, dir_path);
                        self.data = @ptrCast(slice);
                    }
                }.f, struct {
                    fn f(self: *Self, allocator: std.mem.Allocator) void {
                        if (self.data) |data| {
                            const slice: *[]const u8 = @ptrCast(@alignCast(data));
                ...
                .get(struct {
                    fn handler_fn(ctx: *Context) !void {
                        const real_dir: *const []const u8 = @ptrCast(@alignCast(ctx.route.data.?));
                        try FsDir.handler_fn(ctx, dir_path, real_dir);
                    }
                }.handler_fn);
        }
    };
